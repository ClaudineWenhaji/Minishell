...
// -----------------------------
// MiniShell : pipeline complet
// -----------------------------

while (1) {
    line = readline("$ ");                    // Lire la ligne du shell
    if (!line) break;

    pipeline_cmds = parse_pipeline(line);     // Séparer par '|'

    int prev_pipe_read = -1;  // Pour connecter le pipe précédent

    for (int i = 0; i < pipeline_cmds.length; i++) {
        cmd = pipeline_cmds[i];
        tokens = parse_tokens(cmd);
        tokens = expand_variables(tokens);       // $VAR, ~
        redirs = extract_redirections(tokens);   // >, >>, <, <<

        // Sauver stdin/stdout pour restauration
        saved_stdin = dup(STDIN_FILENO);
        saved_stdout = dup(STDOUT_FILENO);

        int pipe_fd[2] = {-1, -1};
        if (i < pipeline_cmds.length - 1) {
            pipe(pipe_fd);  // créer pipe pour la prochaine commande
        }

        // Fork pour chaque commande (builtin ou externe)
        pid = fork();
        if (pid == 0) {
            // ----------------
            // FILS : exécution
            ...
            // Si pipe précédent : connecter stdin
            if (prev_pipe_read != -1) {
                dup2(prev_pipe_read, STDIN_FILENO);
                close(prev_pipe_read);
            }

            // Si pipe suivant : connecter stdout
            if (pipe_fd[1] != -1) {
            ...
            for each redir in redirs {
                switch(redir.type) {
                    case '>':  open file, dup2(fd, STDOUT_FILENO); break;
                    case '>>': open file append, dup2(fd, STDOUT_FILENO); break;
                    case '<':  open file, dup2(fd, STDIN_FILENO); break;
                    case '<<': heredoc_pipe(fd), dup2(fd, STDIN_FILENO); break;
                }
            }

            // BUILTIN ?
            if (is_builtin(tokens[0])) {
                if (!affects_shell(tokens[0])) {
                    builtin_execute(tokens);
                    exit(0);
                } else {
                    // Builtin qui affecte le shell dans pipeline → fork quand même
                    builtin_execute(tokens);
                    exit(0);
                }
            } else {
        ...
        waitpid(pid);  // attendre fin du fils

        // Restaurer stdin/stdout du parent
...
Explications clés
    ...
    Pipeline :
        ...
        Chaque commande est forkée.

        Les pipes sont correctement reliés : stdout du précédent → stdin du suivant.

    Redirections :

        Appliquées dans le fils, avant la commande ou builtin.

        Même pour les builtins qui affectent le shell, dans une pipeline on fork pour isoler la sortie.

    Builtins :

        cd, export, unset, exit : affectent le shell parent, mais si dans une pipeline, le fork est nécessaire pour le flux.

        Les autres builtins peuvent être exécutés directement dans le fils.

    Variables et tilde :

        Expansion faite avant les redirections pour que $VAR > file écrive la valeur correcte.

    Restauration des FD :

        Le parent restaure stdin/stdout après chaque commande pour ne pas polluer la ligne suivante.
